...
def get_trajectories(scores, scores_avg):
    ...
    while not len(rewards) > BATCH_SIZE:
        ...
        while not done:
            ...
            actions.append(action.detach().squeeze().numpy())
...
def update_actor(states_tensor, actions_tensor, advantages_tensor):
    ...
    action_log_probs_old = action_dist_old.log_prob(actions_tensor).detach().sum(1)

    mean, std = actor(states_tensor)
    action_dist = torch.distributions.Normal(mean.squeeze(), std.squeeze())
    action_log_probs = action_dist.log_prob(actions_tensor).sum(1)
...
def plot_graphs(actor_mean, actor_std, loss, loss_critic, i,
    ...
    if i % PLOT_PER == 0:
        ...
        ax_1.scatter(x, y, actor_output_tensor_np[:, 0], marker='.', alpha=0.09, label='action 1')
        ax_1.scatter(x, y, actor_output_tensor_np[:, 1], marker='x', alpha=0.09, label='action 2')
        ...
        # AX 5
        ax_5.cla()
        ax_5.plot(list_state_mean_1, label='m1', marker='.', color='b')
        ax_5.plot(list_state_std_1, label='s1', linestyle='--', color='b')
        ax_5.plot(list_state_mean_2, label='m2', marker='.', color='g')
        ax_5.plot(list_state_std_2, label='s2', linestyle='--', color='g')
